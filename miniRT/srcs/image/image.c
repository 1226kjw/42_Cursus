...
void			my_pixel_put(t_data *data, int x, int y, unsigned int col)
{
	char			*arg;
	
	arg = data->addr + data->bytes_per_line * y + data->bits_per_pixel * x / 8;
	arg[1] = (col >> 16) & 0xff;
	arg[2] = (col >> 8) & 0xff;
	arg[3] = col & 0xff;
}
	...
	while (++j < sd->y)
	{
		...
		while (++i < sd->x)
		{
			...
			if (inter_sp(k, sd->sphere[0]) > 0)
				my_pixel_put(data, i, j, vec_color(sd->sphere[0].color));
			else
				my_pixel_put(data, i, j, C_BG);
		}
	}
	return (0);
}
